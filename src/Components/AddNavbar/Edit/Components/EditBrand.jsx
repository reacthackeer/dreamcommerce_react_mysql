...
import { useUpdateSingleBrandMutation } from '../../../../features/brand/brandApi';
...
const EditBrandComponents = memo(({setSelected}) => {
    ...
    const [previewImages, setPreviewImages] = useState(JSON.parse(localStorage.getItem('edit__brand__images')) || []);
    ...
    const handleDeleteMarkImages = () => {
            ...
            if(images && images?.length){
                ...
                if(res.status === 200 && res.data && res.data?.status__code === 200){
                    ...
                    localStorage.setItem('edit__brand__images',JSON.stringify(newImagesSrcInfo)); 
                }else{
    ...
    const handleUploadAllImages = () => {
        ...
        if(res.status === 200 && res.data && res.data?.status__code === 200){
            ...
            setImages([])
            localStorage.setItem('edit__brand__images',JSON.stringify([...resetPreviewImages, ...images]));
    ...
    const handleDeleteUploadedImages = () => {
        let images = JSON.parse(localStorage.getItem('edit__brand__images'))||[];
        if(images && images?.length){
            ...
            if(res.status === 200 && res.data && res.data?.status__code === 200){
                ...
                localStorage.setItem('edit__brand__images',JSON.stringify([]));
                setPreviewImages([]);
            }else{
                toast.error('There was a server side error!',{duration: 3000})
            }
    ...
    const [provideBrandInfo,{isLoading, isSuccess, isError, data}] = useUpdateSingleBrandMutation();
    const [brandName, setBrandName] = useState(localStorage.getItem('edit__brand__name') || '');
    const handleSubmit = (e) => {
        e.preventDefault(); 
        let images = JSON.parse(localStorage.getItem('edit__brand__images'));
        let brand = brandName;
        let ID = Number(localStorage.getItem('edit__brand__id'));
        let uid = localStorage.getItem('edit__brand__uid');

        if(images && images?.length > 0){
            if(images?.length > 1){
                toast.error('Only one image acceptable!',{duration: 3000});
            }else{
                if(brand && ID && uid && images?.length === 1){
                    let postData = {src: images[0], brand, uid, ID};
    ...
    useEffect(()=>{
        ...
        if(!isError && isSuccess && !isLoading){
            ...
            setImages([]);
            setPreviewImages([]); 
            localStorage.removeItem('edit__brand__images');
            localStorage.removeItem('edit__brand__name');
            localStorage.removeItem('edit__brand__id');
            localStorage.removeItem('edit__brand__uid');
            setSelected(false);
        }
