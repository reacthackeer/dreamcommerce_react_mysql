...
import { useAddSectionMutation } from '../../../features/getAll/api';
...
const AddCategory = memo(() => {
    ...
    const [previewImages, setPreviewImages] = useState(JSON.parse(localStorage.getItem('section__images')) || []);
    ...
    const handleDeleteMarkImages = () => {
            ...
            if(images && images?.length){
                ...
                if(res.status === 200 && res.data && res.data?.status__code === 200){
                    ...
                    localStorage.setItem('section__images',JSON.stringify(newImagesSrcInfo)); 
                }else{
    ...
    const handleUploadAllImages = () => {
        ...
        if(res.status === 200 && res.data && res.data?.status__code === 200){
            ...
            setImages([])
            localStorage.setItem('section__images',JSON.stringify([...resetPreviewImages, ...images]));
    ...
    const handleDeleteUploadedImages = () => {
        let images = JSON.parse(localStorage.getItem('section__images'))||[];
        if(images && images?.length){
            ...
            if(res.status === 200 && res.data && res.data?.status__code === 200){
                ...
                localStorage.setItem('section__images',JSON.stringify([]));
                setPreviewImages([]);
            }else{
                toast.error('There was a server side error!',{duration: 3000})
            }
    ...
    let [provideParentData,{data, isLoading, isSuccess, isError, error}] = useAddSectionMutation();
    ...
    useEffect(()=>{
        ...
        if(!isLoading && isSuccess && !isError){
            if(data && data?.status__code === 201){
                ...
                localStorage.removeItem('section__images');
                // navigate('/');
                setProduct({});
                setPreviewImages([]);
                setImages([]);
    ...
    const handleSubmit = (e) => {
        ...
        let images = JSON.parse(localStorage.getItem('section__images')) || [];
        let uid = numberUid(8);
        let postData = {name: section, src: images[images.length - 1], uid}
        
        if(postData && postData.name && postData.uid){
            if(images?.length === 0){
                toast.error('Please upload image!',{duration: 3000, position: 'top-right'})
            }else{
                if(images.length === 1){ 
                    let items = upNavbarData.items.filter((info)=> info.name.toLowerCase() === postData.name.toLowerCase());
                        if(items.length > 0){
    ...
    return (
        <React.Fragment>
            <DynamicHeader message={'Add Section'}/>
            <DynamicTab/>
            <div className='main__category__product__view__upper__container bg__1'>  
                ...
                <Text fontSize={'2xl'} paddingBottom={'30px'}>Category Basic Information</Text>
                    <form onSubmit={handleSubmit} className='form__item__view__main__container'> 
                        ...
                        {product && upNavbarData && upNavbarData?.items?.length > 0 && 
                        ...
                        <div className='register__button'>
                            <Button 
                                ...
                                size='sm'
                                isLoading={isLoading}
                                isDisabled={!product?.section || previewImages?.length === 0 || previewImages?.length > 1}
                            >
                                Save
                            </Button>  
                        </div>
