...
import { useAddSingleShopByCategoryMutation, useGetAllShopByCategoryQuery } from '../../../features/shopByCategory/shopByCategoryApi';
import DynamicHeader from '../../AddNavbar/DynamicHeader';
import DynamicHeaderHome from '../DynamicHeaderHome';
const ShopByBrand = memo(() => {
    ...
    const [previewImages, setPreviewImages] = useState(JSON.parse(localStorage.getItem('by__category__images')) || []);
    ...
    const handleDeleteMarkImages = () => {
            ...
            if(images && images?.length){
                ...
                if(res.status === 200 && res.data && res.data?.status__code === 200){
                    ...
                    localStorage.setItem('by__category__images',JSON.stringify(newImagesSrcInfo)); 
                }else{
    ...
    const handleUploadAllImages = () => {
        ...
        if(res.status === 200 && res.data && res.data?.status__code === 200){
            ...
            setImages([])
            localStorage.setItem('by__category__images',JSON.stringify([...resetPreviewImages, ...images]));
    ...
    const handleDeleteUploadedImages = () => {
        let images = JSON.parse(localStorage.getItem('by__category__images'))||[];
        if(images && images?.length){
            ...
            if(res.status === 200 && res.data && res.data?.status__code === 200){
                ...
                localStorage.setItem('by__category__images',JSON.stringify([]));
                setPreviewImages([]);
            }else{
                toast.error('There was a server side error!',{duration: 3000})
            }
    ...
    const [provideBrandInfo,{data, isLoading, isError, isSuccess, error}] = useAddSingleShopByCategoryMutation();
    ...
    return (
        <React.Fragment>
            ...
            <Text fontSize={'2xl'} paddingBottom={'30px'}>Shop By category Basic Information</Text>
                <form onSubmit={handleSubmit} className='form__item__view__main__container'> 
                    <FormControl id="brand" isRequired>
                        <FormLabel>Name</FormLabel>
                        <Input
                        type="text"
                        placeholder='Enter by category name'
                        name="name"
                        value={product.name}
                    ...
                    <FormControl id="brand" isRequired>
                        ...
                        <Input
                        type="text"
                        placeholder='Enter by category link'
                        name="link"
                        value={product.link}
                        onChange={handleChange}
                        />
