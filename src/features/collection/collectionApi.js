...
export const offerApi = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        getAllCollectionProduct: builder.query({
            query: ({father, parent, child, page, limit}) => `/products/p/${father}/${parent}/${child}?page=${page}&&peerPage=${limit}`,
            providesTags: () => [{type: 'get__all__child__product'}]
        }),
        getAllCollectionSimilarProduct: builder.query({
            query: ({father, parent, child, product__id, page, limit}) => `/products/p/similar__product/${father}/${parent}/${child}/${product__id}?page=${page}&&peerPage=${limit}`
        })
    })
})
