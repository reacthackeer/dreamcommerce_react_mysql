...
const AddSimilarProduct = () => {
  ...
  if(!isLoading && isSuccess && !isError){
      if(data.status__code === 200){
          ...
          let {current__price, previous__price, title, overviews, images, specifications, details} = infos;

            title = title.split(' ');
            title = title.slice(1, title.length).join(' '); 
          let newProductInfo = {ID, brand, up, child, parent, parent__father, product__id, quantity, total__sell, views, visible, current__price, previous__price, title, wholesale__price: infos.wholesale__price || 0};
...
export default AddSimilarProduct;
