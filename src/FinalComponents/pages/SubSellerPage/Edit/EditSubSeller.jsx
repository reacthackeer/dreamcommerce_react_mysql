...
import EditSellerComponents from './Components/EditSellerComponents';

const EditSubSeller = () => { 
    const {auth} = useSelector((state)=> state.auth);
    const {isLoggedIn, role, store__email, store__id, designation} = auth;
    let queryData = {role: '4', designation: 'subSeller', store__id: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__id, store__email: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__email};
    ...
    return (
        ...
        : 
                <EditSellerComponents setSelected={setSelected}/>
        }
        </AdminPageSkeleton>
    );
};

export default EditSubSeller;