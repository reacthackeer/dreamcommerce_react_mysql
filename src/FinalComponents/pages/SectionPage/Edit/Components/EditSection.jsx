...
import { useUpdateSingleUpMutation } from '../../../../../features/brand/brandApi';
const EditSectionComponents = memo(({setSelected}) => {
    const [images, setImages] = useState([]);
    const [previewImages, setPreviewImages] = useState(JSON.parse(localStorage.getItem('edit__section__images')) || []);
    ...
    const handleDeleteMarkImages = () => {
            ...
            if(images && images?.length){
                ...
                if(res.status === 200 && res.data && res.data?.status__code === 200){
                    ...
                    localStorage.setItem('edit__section__images',JSON.stringify(newImagesSrcInfo)); 
                }else{
    ...
    const handleUploadAllImages = () => {
        ...
        if(res.status === 200 && res.data && res.data?.status__code === 200){
            ...
            setImages([])
            localStorage.setItem('edit__section__images',JSON.stringify([...resetPreviewImages, ...images]));
    ...
    const handleDeleteUploadedImages = () => {
        let images = JSON.parse(localStorage.getItem('edit__section__images'))||[];
        if(images && images?.length){
            ...
            if(res.status === 200 && res.data && res.data?.status__code === 200){
                ...
                localStorage.setItem('edit__section__images',JSON.stringify([]));
                setPreviewImages([]);
            }else{
                toast.error('There was a server side error!',{duration: 3000})
            }
    ...
    const [topCategoryName, setTopCategoryName] = useState(localStorage.getItem('edit__section__name') || '');
    const [provideParentInfo, {data, isLoading, isError, isSuccess}] = useUpdateSingleUpMutation();
    const handleSubmit = (e) => {
        e.preventDefault();   

        let images = JSON.parse(localStorage.getItem('edit__section__images'));
        let name = topCategoryName;
        let ID = localStorage.getItem('edit__section__id');
        let uid = localStorage.getItem('edit__section__uid'); 
    ...
    useEffect(()=>{
        ...
        if(!isError && isSuccess && !isLoading){
            ...
            setImages([]);
            setPreviewImages([]); 
            localStorage.removeItem('edit__section__images');
            localStorage.removeItem('edit__section__name');
            localStorage.removeItem('edit__section__id');
            localStorage.removeItem('edit__section__uid'); 
            setSelected(false);
        }
...
export default EditSectionComponents;