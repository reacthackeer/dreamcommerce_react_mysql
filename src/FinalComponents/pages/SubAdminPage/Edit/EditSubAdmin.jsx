...
import EditSubAdminComponents from './Components/EditSubAdmin';

const EditSubAdmin = () => { 
    const {auth} = useSelector((state)=> state.auth);
    const {isLoggedIn, role, store__email, store__id, designation} = auth;
    let queryData = {role: '2', designation: 'subAdmin', store__id: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__id, store__email: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__email};
    ...
    return (
        ...
        : 
                <EditSubAdminComponents setSelected={setSelected}/>
        }
        </AdminPageSkeleton>
    );
};
