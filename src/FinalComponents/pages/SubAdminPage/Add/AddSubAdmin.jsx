...
const AddSubAdmin = memo(() => {
    const {auth} = useSelector((state)=> state.auth); 
    const [product, setProduct] = useState({store__id: auth && auth?.designation === 'subAdmin' && auth?.role === 2 ? auth.store__id : '', store__email: auth && auth?.designation === 'subAdmin' && auth?.role === 2 ? auth.store__email : ''});
    ...
    const handleSubmit = (e) => {
        ...
        if(product.password === product.confirm__password){
            let userInfo = {...product, user__id: uid(13), role: 2, designation: 'subAdmin'};
    ...
    useEffect(()=>{
        ...
        if(!isError && !isLoading && isSuccess){
            ...
            if(data?.status__code === 201){
                ...
                setProduct({store__id: auth && auth?.designation === 'subAdmin' && auth?.role === 2 ? auth.store__id : '', store__email: auth && auth?.designation === 'subAdmin' && auth?.role === 2 ? auth.store__email : ''});
