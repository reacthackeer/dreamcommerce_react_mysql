...
import { useUpdateSingleShopByBrandMutation } from '../../../../../features/shopByBrand/shopByBrandApi';

const EditShopByBrandComponent = memo(({setSelected}) => {
    
    
    const [images, setImages] = useState([]);
    const [previewImages, setPreviewImages] = useState(JSON.parse(localStorage.getItem('edit__shop__by__brand__images')) || []);
    ...
    const handleDeleteMarkImages = () => {
            ...
            if(images && images?.length){
                ...
                if(res.status === 200 && res.data && res.data?.status__code === 200){
                    ...
                    localStorage.setItem('edit__shop__by__brand__images',JSON.stringify(newImagesSrcInfo)); 
                }else{
    ...
    const handleUploadAllImages = () => {
        ...
        if(res.status === 200 && res.data && res.data?.status__code === 200){
            ...
            setImages([])
            localStorage.setItem('edit__shop__by__brand__images',JSON.stringify([...resetPreviewImages, ...images]));
    ...
    const handleDeleteUploadedImages = () => {
        let images = JSON.parse(localStorage.getItem('edit__shop__by__brand__images'))||[];
        if(images && images?.length){
            ...
            if(res.status === 200 && res.data && res.data?.status__code === 200){
                ...
                localStorage.setItem('edit__shop__by__brand__images',JSON.stringify([]));
                setPreviewImages([]);
            }else{
                toast.error('There was a server side error!',{duration: 3000})
            }
    ...
    const [provideBrandInfo,{isLoading, isSuccess, isError, data}] = useUpdateSingleShopByBrandMutation();
    const [popularCategoryName, setPopularCategoryName] = useState(localStorage.getItem('edit__shop__by__brand__name') || ''); 
    const [popularCategoryLink, setPopularCategoryLink] = useState(localStorage.getItem('edit__shop__by__brand__link') || ''); 
    const handleSubmit = (e) => {
        e.preventDefault(); 
        let images = JSON.parse(localStorage.getItem('edit__shop__by__brand__images'));
        let name = popularCategoryName;
        let link = popularCategoryLink;
        let id = Number(localStorage.getItem('edit__shop__by__brand__id')); 
    ...
    useEffect(()=>{
        ...
        if(!isError && isSuccess && !isLoading){
            ...
            setImages([]);
            setPreviewImages([]); 
            localStorage.removeItem('edit__shop__by__brand__images');
            localStorage.removeItem('edit__shop__by__brand__name');
            localStorage.removeItem('edit__shop__by__brand__id');
            localStorage.removeItem('edit__shop__by__brand__link');
            setSelected(false);
        }
                    ...
                    <Box className='data__view__form'>
                        <FormControl id="image">
                            ...
                            <Input
                                ...
                                placeholder='Shop by brand name....'
                        ...
                        <FormControl id="image">
                            ...
                            <Input
                                ...
                                placeholder='Shop by brand link....'
...
export default EditShopByBrandComponent;