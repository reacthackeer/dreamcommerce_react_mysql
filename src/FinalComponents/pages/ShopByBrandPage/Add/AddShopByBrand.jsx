...
import { useAddSingleShopByBrandMutation, useGetAllShopByBrandQuery } from '../../../../features/shopByBrand/shopByBrandApi';
import AdminPageSkeleton from '../../AdminPageSkeletonComponents/AdminPageSkeleton';
const AddShopByBrand = memo(() => {
    ...
    const [previewImages, setPreviewImages] = useState(JSON.parse(localStorage.getItem('by__brand__images')) || []);
    ...
    const handleDeleteMarkImages = () => {
            ...
            if(images && images?.length){
                ...
                if(res.status === 200 && res.data && res.data?.status__code === 200){
                    ...
                    localStorage.setItem('by__brand__images',JSON.stringify(newImagesSrcInfo)); 
                }else{
    ...
    const handleUploadAllImages = () => {
        ...
        if(res.status === 200 && res.data && res.data?.status__code === 200){
            ...
            setImages([])
            localStorage.setItem('by__brand__images',JSON.stringify([...resetPreviewImages, ...images]));
    ...
    const handleDeleteUploadedImages = () => {
        let images = JSON.parse(localStorage.getItem('by__brand__images'))||[];
        if(images && images?.length){
            ...
            if(res.status === 200 && res.data && res.data?.status__code === 200){
                ...
                localStorage.setItem('by__brand__images',JSON.stringify([]));
                setPreviewImages([]);
            }else{
                toast.error('There was a server side error!',{duration: 3000})
            }
    ...
    const [provideBrandInfo,{data, isLoading, isError, isSuccess, error}] = useAddSingleShopByBrandMutation();
                    ...
                    <Box className='data__view__form'>
                        <FormControl id="brand" isRequired>
                            ...
                            <Input
                                size='sm'
                                type="text"
                                placeholder='Enter by brand name'
                                name="name"
                                value={product.name}
                        ...
                        <FormControl id="brand" isRequired>
                            ...
                            <Input
                                size='sm'
                                type="text"
                                placeholder='Enter by brand link'
                                name="link"
                                value={product.link}
                                onChange={handleChange}
                            />
