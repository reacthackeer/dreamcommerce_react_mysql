...
import EditCourierComponent from './Components/EditCourerComponent';
const EditCourier = () => { 
    const {auth} = useSelector((state)=> state.auth);
    const {isLoggedIn, role, store__email, store__id, designation} = auth;
    let queryData = {role: '7', designation: 'courier', store__id: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__id, store__email: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__email};
    ...
    return (
        ...
        : 
                <EditCourierComponent setSelected={setSelected}/>
        }
        </AdminPageSkeleton>
    );
};
