...
import EditSubModeratorComponents from './Components/EditSubModeratorComponents';

const EditSubModerator = () => { 
    const {auth} = useSelector((state)=> state.auth);
    const {isLoggedIn, role, store__email, store__id, designation} = auth;
    let queryData = {role: '6', designation: 'subModerator', store__id: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__id, store__email: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__email};
    ...
    return (
        ...
        : 
                <EditSubModeratorComponents setSelected={setSelected}/>
        }
        </AdminPageSkeleton>
    );
};

export default EditSubModerator;