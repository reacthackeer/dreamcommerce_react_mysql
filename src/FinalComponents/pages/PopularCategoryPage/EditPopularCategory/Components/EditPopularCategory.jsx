...
import { useUpdateSinglePopularCategoryMutation } from '../../../../../features/popularCategory/popularCategoryApi';
    

const EditBrandComponents = memo(({setSelected}) => {
    
    
    const [images, setImages] = useState([]);
    const [previewImages, setPreviewImages] = useState(JSON.parse(localStorage.getItem('edit__popular__category__images')) || []);
    ...
    const handleDeleteMarkImages = () => {
            ...
            if(images && images?.length){
                ...
                if(res.status === 200 && res.data && res.data?.status__code === 200){
                    ...
                    localStorage.setItem('edit__popular__category__images',JSON.stringify(newImagesSrcInfo)); 
                }else{
    ...
    const handleUploadAllImages = () => {
        ...
        if(res.status === 200 && res.data && res.data?.status__code === 200){
            ...
            setImages([])
            localStorage.setItem('edit__popular__category__images',JSON.stringify([...resetPreviewImages, ...images]));
    ...
    const handleDeleteUploadedImages = () => {
        let images = JSON.parse(localStorage.getItem('edit__popular__category__images'))||[];
        if(images && images?.length){
            ...
            if(res.status === 200 && res.data && res.data?.status__code === 200){
                ...
                localStorage.setItem('edit__popular__category__images',JSON.stringify([]));
                setPreviewImages([]);
            }else{
                toast.error('There was a server side error!',{duration: 3000})
            }
    ...
    const [provideBrandInfo,{isLoading, isSuccess, isError, data}] = useUpdateSinglePopularCategoryMutation();
    const [popularCategoryName, setPopularCategoryName] = useState(localStorage.getItem('edit__popular__category__name') || ''); 
    const [popularCategoryLink, setPopularCategoryLink] = useState(localStorage.getItem('edit__popular__category__link') || ''); 
    const handleSubmit = (e) => {
        e.preventDefault(); 
        let images = JSON.parse(localStorage.getItem('edit__popular__category__images'));
        let name = popularCategoryName;
        let link = popularCategoryLink;
        let id = Number(localStorage.getItem('edit__popular__category__id')); 
    ...
    useEffect(()=>{
        ...
        if(!isError && isSuccess && !isLoading){
            ...
            setImages([]);
            setPreviewImages([]); 
            localStorage.removeItem('edit__popular__category__images');
            localStorage.removeItem('edit__popular__category__name');
            localStorage.removeItem('edit__popular__category__id');
            localStorage.removeItem('edit__popular__category__link');
            setSelected(false);
        }
                ...
                <form onSubmit={handleSubmit}> 
                    <Box className='data__view__form'>
                        <FormControl id="image">
                            ...
                            placeholder='Popular category name....'
                        ...
                        <FormControl id="image">
                            ...
                            placeholder='Popular category link....'
...
export default EditBrandComponents;