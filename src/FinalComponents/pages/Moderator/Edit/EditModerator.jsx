...
import EditModeratorComponents from './Components/EditModeratorComponents';

const EditModerator = () => { 
    const {auth} = useSelector((state)=> state.auth);
    const {isLoggedIn, role, store__email, store__id, designation} = auth;
    let queryData = {role: '5', designation: 'moderator', store__id: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__id, store__email: (isLoggedIn && role === 1 && designation === 'admin') ? '*' : store__email};
    ...
    return (
        ...
        : 
                <EditModeratorComponents setSelected={setSelected}/>
        }
        </AdminPageSkeleton>
    );
};
