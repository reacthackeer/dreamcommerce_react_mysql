...
import { useUpdateSingleChildMutation } from '../../../../../features/brand/brandApi';

const EditCollectionComponents = memo(({setSelected}) => {
    const [images, setImages] = useState([]);
    const [previewImages, setPreviewImages] = useState(JSON.parse(localStorage.getItem('edit__collection__images')) || []);
    ...
    const handleDeleteMarkImages = () => {
            ...
            if(images && images?.length){
                ...
                if(res.status === 200 && res.data && res.data?.status__code === 200){
                    ...
                    localStorage.setItem('edit__collection__images',JSON.stringify(newImagesSrcInfo)); 
                }else{
    ...
    const handleUploadAllImages = () => {
        ...
        if(res.status === 200 && res.data && res.data?.status__code === 200){
            ...
            setImages([])
            localStorage.setItem('edit__collection__images',JSON.stringify([...resetPreviewImages, ...images]));
    ...
    const handleDeleteUploadedImages = () => {
        let images = JSON.parse(localStorage.getItem('edit__collection__images'))||[];
        if(images && images?.length){
            ...
            if(res.status === 200 && res.data && res.data?.status__code === 200){
                ...
                localStorage.setItem('edit__collection__images',JSON.stringify([]));
                setPreviewImages([]);
            }else{
                toast.error('There was a server side error!',{duration: 3000})
            }
    ...
    const [provideChildInfo, {data, isLoading, isError, isSuccess}] = useUpdateSingleChildMutation();
    const [collectionName, setCollectionName] = useState(localStorage.getItem('edit__collection__name') || '');
    const handleSubmit = (e) => {
        e.preventDefault();   

        let images = JSON.parse(localStorage.getItem('edit__collection__images'));
        let name = collectionName;
        let ID = localStorage.getItem('edit__collection__id');
        let uid = Number(localStorage.getItem('edit__collection__uid'));
        let up = localStorage.getItem('edit__collection__up');
        let parent = localStorage.getItem('edit__collection__parent');
        if(images && images?.length > 0){
            if(images?.length > 1){
                toast.error('Only one image acceptable!',{duration: 3000});
            }else{
                if(name && ID && uid && up && parent && images?.length === 1){
                    let postData = {src: images[0], name, uid, ID, up, parent};
                    provideChildInfo(postData);
                }else{
    ...
    useEffect(()=>{
        ...
        if(!isError && isSuccess && !isLoading){
            ...
            setImages([]);
            setPreviewImages([]); 
            localStorage.removeItem('edit__collection__images');
            localStorage.removeItem('edit__collection__name');
            localStorage.removeItem('edit__collection__id');
            localStorage.removeItem('edit__collection__uid');
            localStorage.removeItem('edit__collection__up');
            localStorage.removeItem('edit__collection__parent');
            setSelected(false);
        }
...
export default EditCollectionComponents;